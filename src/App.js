...
// Panel visibility states - camera preview always visible, control panel only on desktop, debug enabled
  ...
  const [showCarousel, setShowCarousel] = useState(true) // Model carousel toggle (desktop only)
  ...
  return (
    <div style={{
      ...
      {/* Model Carousel Toggle Button - Desktop only */}
      {!isMobile && (
        <button
          onClick={() => setShowCarousel(!showCarousel)}
          style={{
            position: 'fixed',
            bottom: showCarousel ? '180px' : '10px',
            right: '10px',
            padding: '10px 16px',
            fontSize: '12px',
            backgroundColor: showCarousel ? 'rgba(59, 130, 246, 0.9)' : 'rgba(100, 200, 100, 0.9)',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            cursor: 'pointer',
        ...
        >
          ...
          {showCarousel ? 'Hide Models' : 'Show Models'}
        </button>
      )}
    </div>
  )
